...
class Proceso():
	def __init__(self,programador="",operacion="0/0",tiempoMaximoEstimado=TIEMPO_ESTIMADO_SEGUNDOS_INICIAL,noPrograma=NO_PROGRAMA_INICIAL,tiempoBloqueado = TIEMPO_BLOQUEADO) -> None:
		self.__programador = ""
		self._operacion = "0/0"
		self._tiempoEstimadoSegundos = TIEMPO_ESTIMADO_SEGUNDOS_INICIAL
		self._noPrograma = NO_PROGRAMA_INICIAL
		self._tiempoBloqueado = TIEMPO_BLOQUEADO
		...
		self.fijaTiempoBloqueado(tiempoBloqueado)
	...
	def fijaTiempoBloqueado(self,tiempoBloqueado):
		valida = tiempoBloqueado >= 0
		if valida:
			self._tiempoBloqueado = int(tiempoBloqueado)
		return valida
	...
	def dameTiempoBloqueado(self) -> int:
		return self._tiempoBloqueado
